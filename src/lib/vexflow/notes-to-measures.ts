...
function quantizedDurationSplit(beats: number, isRest: boolean): StaveDescriptor[] {
  const descriptors: StaveDescriptor[] = []
  let remaining = beats
  while (remaining > 0.001) {
    const code = isRest ? beatsToRestDuration(remaining) : beatsToDuration(remaining)
...
export function notesToMeasures(
  ...
  for (let m = 0; m < measureCount; m++) {
    ...
    const measureNoteDescs = buildMeasureNotes(groups, measureStart, beatsPerMeasure)
