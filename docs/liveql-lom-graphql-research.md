...
```graphql
{ live_set { track(index: 0) { clip_slot(index: 0) { clip {
...
```graphql
mutation { clip_get_notes_extended(id: 42, from_pitch: 0, pitch_span: 128, from_time: 0, time_span: 16.0) {
  notes { note_id pitch start_time duration velocity }
} }
```
...
```graphql
mutation { clip_apply_note_modifications(id: 42, notes_dictionary: {
  notes: [
    { note_id: 14, pitch: 62, start_time: 0.0, duration: 1.5, velocity: 110 }
  ]
}) { id notes { note_id pitch start_time duration velocity } } }
```
...
mutation { song_stop_playing(id: 1) { is_playing } }
```
...
```graphql
mutation { clip_fire(id: 42) { id name is_playing } }
```
...
{ live_set {
  tracks {
    id name has_midi_input
    clip_slots {
      id has_clip
      clip { id name length is_midi_clip looping }
    }
  }
} }
```
